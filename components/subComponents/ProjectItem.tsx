...
const ProjectItem = ( {bgUrl, appName, tecs, description, webPage, gitUrl}:ProjectData ) => {
  ...
  return (
    ...
    <div className='relative group w-full h-72 justify-center items-center text-white text-center'>
        ...
        <div className='z-10 h-full w-full flex flex-col gap-2 bg-[#1D3557]/70 items-center justify-center opacity-0 group-hover:opacity-100
        transition-all ease-in-out backdrop-blur gap-y-8 p-3 overflow-hidden ts'>
            <div className='w-full -translate-y-6 transition ease-in-out group-hover:-translate-y-0 scale-125 group-hover:scale-100'>
                ...
                <p className='text-sky-400 text-lg'>{tecs}</p>
                <p className='serif-pro font-extralight mt-4 scale-125 group-hover:scale-100 transition-all ease-in-out'>{description}</p>
            </div>
            <div className='flex gap-x-3 w-full transition ease-in-out scale-125 group-hover:scale-100 translate-y-6 group-hover:translate-y-0
            justify-center'>
                ...
                {
                gitUrl && <a className='px-4 py-2 border hover:bg-[rgba(145,13,13,.8)] transition ease-in-out pointer-events-none group-hover:pointer-events-auto' href={gitUrl} target="_blank">
